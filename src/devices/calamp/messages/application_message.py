...
from devices.calamp.api import AppMessageTypes
...
class ApplicationMessage(CalampMessageBase):
	...
	def parse(self, buffer):
		...
		self.app_msg_type = struct.unpack(">h", self.buffer.read(cf.APP_MSG_TYPE_LEN))[0]
		# self.parse_app_msg()
   
	def parse_app_msg(self):
		"""TODO: App message is post processed after parsing differently depending on the app msg type """
		self.app_msg_len = struct.unpack(">h", self.buffer.read(cf.APP_MSG_LEN))[0]
		for i in range(self.app_msg_len):
			self.app_msg += struct.unpack(">h", self.buffer.read())[0]
	...
	def log(self, log):
		...
		log.debug("app msg type: {} ({})".format(self.app_msg_type, 2))
		log.debug("app msg: {}".format(self.app_msg))